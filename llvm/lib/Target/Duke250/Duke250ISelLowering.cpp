//===-- Duke250ISelLowering.cpp - Duke250 DAG Lowering Implementation -----------===//
//
...
#define DEBUG_TYPE "Duke250-isellower"

#include "Duke250GenCallingConv.inc"

Duke250TargetLowering::Duke250TargetLowering(const TargetMachine &TM,
                                         const Duke250Subtarget &STI)
    : TargetLowering(TM), Subtarget(STI)
{
  // Set up the register classes
  addRegisterClass(MVT::i16, &Duke250::GPRRegClass);
  ...
  setStackPointerRegisterToSaveRestore(Duke250::r6);
...
const char *Duke250TargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case Duke250ISD::Ret: return "Duke250ISD::Ret";
  default:            return NULL;
  }
}

void Duke250TargetLowering::ReplaceNodeResults(SDNode *N,
...
static const MCPhysReg GPRArgRegs[] = {
  Duke250::r1, Duke250::r2, Duke250::r3, Duke250::r4
};

/// LowerFormalArguments - transform physical registers into virtual registers
/// and generate load operations for arguments places on the stack.
SDValue Duke250TargetLowering::LowerFormalArguments(
                                    SDValue Chain,
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, Duke250_CCallingConv);
  ...
  unsigned ArgRegBegin = Duke250::r1;
...
bool Duke250TargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                MachineFunction &MF, bool isVarArg,
                                const SmallVectorImpl<ISD::OutputArg> &Outs,
                                LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, isVarArg, MF, RVLocs, Context);
  return CCInfo.CheckReturn(Outs, Duke250_CRetConv);
}

SDValue Duke250TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                       SmallVectorImpl<SDValue> &InVals) const {
  llvm_unreachable("Cannot lower call");
}

SDValue
Duke250TargetLowering::LowerReturn(SDValue Chain,
  ...
  CCInfo.AnalyzeReturn(Outs, Duke250_CRetConv);
  ...
  return DAG.getNode(Duke250ISD::Ret, dl, MVT::Other, RetOps);
}
...
SDValue
Duke250TargetLowering::LowerGlobalAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported global address");
}

SDValue
Duke250TargetLowering::LowerBlockAddress(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported block address");
}

SDValue
Duke250TargetLowering::LowerConstantPool(SDValue Op, SelectionDAG &DAG) const {
  llvm_unreachable("Unsupported constant pool");
}

SDValue
Duke250TargetLowering::LowerRETURNADDR(SDValue Op, SelectionDAG &DAG) const {
  return SDValue();
}

SDValue
Duke250TargetLowering::LowerShlParts(SDValue Op, SelectionDAG &DAG) const {
...
SDValue
Duke250TargetLowering::LowerShrParts(SDValue Op, SelectionDAG &DAG,
                                   bool arith) const {
  assert(Op.getNumOperands() == 3 && "Not a long shift");
...
SDValue
Duke250TargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
